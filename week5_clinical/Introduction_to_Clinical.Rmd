---
title: "Intro to TCGA Clinical Data COMPLETED"
date: 07/17/22
updated: 09/26/24
---
...
Let's explore the built in mtcars data frame before we get to using the TCGA data frame to learn about this crucial data type.
...
```{r Exercise 4.1}
# create plot here
plot(clinic$lymph_nodes_examined_count, clinic$death_days_to, main = "Lymph Node Count vs. Days until Death", xlab = "Count of Examined Lymph Nodes",ylab = "Days until Death", col = 3)
```
...
boxplot(clinic$lymph_nodes_examined_count ~ clinic$gender, main = "Lymph Node Count vs. Gender", xlab = "Gender",ylab = "Lymph Node Count", col = 3)
...
0.016, not statistically significant, the relationship between age and breast cancer survival is not well defined due to right censoring (lack of data).
